...
class ApiService {
  ...
  /// Search buyers by brand name (web "Top brands" table).
  /// Endpoint: POST /searchBuyers with JSON body: {"brandname": "..."}
  Future<ApiResponse<List<BrandBuyerItem>>> searchBuyersByBrand({
    required String brandName,
  }) async {
    try {
      final response = await _dio.post(
        'searchBuyers',
        data: json.encode({'brandname': brandName}),
        options: Options(
      ...
      if (response.statusCode == 200) {
        ...
        final items = <BrandBuyerItem>[];
        if (raw is List) {
          for (final e in raw) {
            if (e is Map) {
              items.add(BrandBuyerItem.fromJson(e.cast<String, dynamic>()));
            }
          }
        }

        return ApiResponse<List<BrandBuyerItem>>(
          status: status,
